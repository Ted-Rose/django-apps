...
# All scheduled-job infrastructure is commented out to save costs:
# the Cloud Scheduler triggers, the Cloud Run jobs, their IAM
# bindings, and the scheduler service account. Uncomment to
# re-enable. DB migrations are unaffected — deploy.yml runs them via
# a one-off django-migrate-<sha> Cloud Run job on each deploy.
